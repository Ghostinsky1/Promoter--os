...
const ROLE_CONFIG = {
  headliner: { bg: 'bg-[#C4FF0D]/20', border: 'border-[#C4FF0D]/30', text: 'text-[#C4FF0D]', label: 'HEADLINER', Icon: Star },
...
export function SupportActCard({ act, index, total, onUpdate, onRemove, onMove }: SupportActCardProps) {
  ...
  return (
    <div className={`bg-[#1A1F1E] border ${config.border} rounded-2xl overflow-hidden transition-all`}>
      <div
        className="p-4 md:p-5 cursor-pointer hover:bg-[#141716] transition-colors"
        ...
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 text-xs text-gray-400">
              ...
              <span className="text-[#C4FF0D] font-semibold">{formatCurrency(act.guarantee)}</span>
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            <button type="button" onClick={(e) => { e.stopPropagation(); onMove('up'); }} disabled={index === 0}
              className="p-1.5 hover:bg-[#141716] rounded-lg transition-colors disabled:opacity-30">
      ...
      {expanded && (
        <div className="px-4 md:px-5 pb-5 space-y-5 border-t border-gray-700/50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-5">
            <div>
              ...
              <input type="text" value={act.name} onChange={(e) => onUpdate('name', e.target.value)} placeholder="Artist name"
                className="w-full px-3 py-2.5 bg-[#141716] border border-gray-700 text-white text-sm rounded-xl focus:ring-2 focus:ring-[#C4FF0D] focus:border-[#C4FF0D]" />
            </div>
            <div>
          ...
          {/* Deal Structure */}
          <div className="bg-[#141716] rounded-xl p-4 space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                ...
                <select value={act.deal_type || 'flat_fee'} onChange={(e) => onUpdate('deal_type', e.target.value)}
                  className="w-full px-3 py-2.5 bg-[#0F1113] border border-gray-700 text-white text-sm rounded-xl focus:ring-2 focus:ring-[#C4FF0D] focus:border-[#C4FF0D]">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="number" value={act.guarantee || ''} onChange={(e) => onUpdate('guarantee', parseFloat(e.target.value) || 0)} placeholder="0" min="0"
                    className="w-full pl-7 pr-3 py-2.5 bg-[#0F1113] border border-gray-700 text-white text-sm rounded-xl focus:ring-2 focus:ring-[#C4FF0D] focus:border-[#C4FF0D]" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#141716] rounded-xl p-4 space-y-3">
            ...
            {act.guarantee > 0 && (
              <div className="p-3 bg-[#C4FF0D]/10 border border-[#C4FF0D]/20 rounded-lg mt-2">
                <div className="flex justify-between items-center text-xs">
                  <span className="text-gray-400">Total Payment</span>
                  <span className="text-[#C4FF0D] font-bold">{formatCurrency(act.guarantee)}</span>
                </div>
          ...
          {/* Accommodations */}
          <div className="bg-[#141716] rounded-xl p-4 space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="number" value={act.hotel_budget || ''} onChange={(e) => onUpdate('hotel_budget', Number(e.target.value) || 0)} placeholder="0"
                      className="w-full pl-7 pr-3 py-2 bg-[#0F1113] border border-gray-700 text-white text-xs rounded-lg focus:ring-1 focus:ring-[#C4FF0D]" />
                  </div>
                </div>
                <div>
                  ...
                  <input type="number" value={act.hotel_nights || 1} onChange={(e) => onUpdate('hotel_nights', Number(e.target.value) || 1)} min="1"
                    className="w-full px-3 py-2 bg-[#0F1113] border border-gray-700 text-white text-xs rounded-lg focus:ring-1 focus:ring-[#C4FF0D]" />
                </div>
                <div>
    ...
    <button
      ...
      className={`p-3 rounded-xl border-2 text-center transition-all ${
        active
          ...
          : 'bg-[#0F1113] border-gray-700 hover:border-gray-600'
      }`}
    >
